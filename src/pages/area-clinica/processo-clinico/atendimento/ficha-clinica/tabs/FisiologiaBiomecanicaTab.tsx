...
export interface FisiologiaBiomecanicaTabProps {
  utenteId: string
}

export function FisiologiaBiomecanicaTab({ utenteId }: FisiologiaBiomecanicaTabProps) {
  const { data, isLoading } = useQuery({
    queryKey: ['ficha-clinica-secao-template', 'FISIOLOGIA_BIOMECANICA'],
    queryFn: async () => {
      const client = FichaClinicaSecoesService()
      const res = await client.getTemplates('FISIOLOGIA_BIOMECANICA')
  ...
  const separador = useMemo(() => {
    ...
    return templates.find((t) => t.codigo === 'FISIOLOGIA_BIOMECANICA' && t.ativo) ?? null
  }, [data])
  ...
  if (isLoading) {
    return (
      <p className='text-sm text-muted-foreground'>
        A carregar configuração de Fisiologia/Biomecânica…
      </p>
    )
  }
  ...
  if (!separador) {
    return (
      <p className='text-sm text-muted-foreground'>
        ...
        <span className='font-mono'>FISIOLOGIA_BIOMECANICA</span>. Crie-o em &quot;Separadores
