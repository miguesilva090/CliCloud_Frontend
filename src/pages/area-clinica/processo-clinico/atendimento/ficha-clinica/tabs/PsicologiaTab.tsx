...
export interface PsicologiaTabProps {
  utenteId: string
}

export function PsicologiaTab({ utenteId }: PsicologiaTabProps) {
  const { data, isLoading } = useQuery({
    queryKey: ['ficha-clinica-secao-template', 'PSICOLOGIA'],
    queryFn: async () => {
      const client = FichaClinicaSecoesService()
      const res = await client.getTemplates('PSICOLOGIA')
  ...
  const separador = useMemo(() => {
    ...
    return templates.find((t) => t.codigo === 'PSICOLOGIA' && t.ativo) ?? null
  }, [data])
  ...
  if (isLoading) {
    return (
      <p className='text-sm text-muted-foreground'>
        A carregar configuração de Psicologia…
      </p>
    )
  }
  ...
  if (!separador) {
    return (
      <p className='text-sm text-muted-foreground'>
        ...
        <span className='font-mono'>PSICOLOGIA</span>. Crie-o em &quot;Separadores
