...
import { useFetchAllConcelhosForPrint } from '../../queries/concelhos-queries'
...
export function ConcelhosTable({
  ...
  const { handlePrint, isPrinting } = useReportPrint('listagem-concelhos')
  const [isFetchingAll, setIsFetchingAll] = useState(false)
  const [isPrintOptionsOpen, setIsPrintOptionsOpen] = useState(false)
  const fetchAllConcelhosForPrint = useFetchAllConcelhosForPrint()
  ...
  const onPrintAll = async () => {
    ...
    try {
      ...
      const allConcelhos = await fetchAllConcelhosForPrint({
        filters: state.filters,
        sorting: state.sorting,
      })
      handlePrint(allConcelhos)
    } catch (error) {
      console.error('Error fetching all concelhos for print:', error)
      toast.error('Erro ao buscar todos os concelhos para impressão')
  ...
  return (
    <>
      <AppLoader
        ...
        title={
          isFetchingAll ? 'A buscar concelhos...' : 'A preparar impressão...'
        }
        description={
          isFetchingAll
            ? 'Aguarde enquanto carregamos todos os concelhos'
            : 'Por favor aguarde'
        }
        icon='spinner'
        hideDelay={200}
