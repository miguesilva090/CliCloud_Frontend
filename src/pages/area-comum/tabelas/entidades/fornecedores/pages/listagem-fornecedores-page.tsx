...
import { ListagemFornecedoresTable } from '../components/listagem-fornecedores-table'
import { ListagemFornecedoresFilterControls } from '../components/listagem-fornecedores-filter-controls'
import {
  useGetFornecedoresPaginated,
  usePrefetchAdjacentFornecedores,
} from '../queries/listagem-fornecedores-queries'
...
export function ListagemFornecedoresPage() {
  ...
  const handleOpenDelete = (item: FornecedorTableDTO) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await FornecedorService().deleteFornecedor(String(id))
      ...
      if (status === ResponseStatus.Success) {
        ...
        const msg =
          (response.info as { messages?: Record<string, string[]> })
            ?.messages?.['$']?.[0] ?? 'Falha ao eliminar Fornecedor.'
        toast.error(msg)
      }
    } catch (error: unknown) {
      const err = error as { message?: string }
      toast.error(
        err?.message ?? 'Ocorreu um erro ao eliminar o Fornecedor.'
      )
    } finally {
      setIsDeleting(false)
    }
      ...
      <DashboardPageContainer>
        ...
        {isError ? (
          <Alert variant='destructive' className='mb-4'>
            ...
            <AlertDescription>
              {errorMessage ||
                'Ocorreu um erro ao pedir a lista de fornecedores.'}
            </AlertDescription>
          </Alert>
        ) : null}

        <ListagemFornecedoresTable
          data={fornecedores}
          isLoading={isLoading}
          pageCount={pageCount}
          totalRows={totalRows}
