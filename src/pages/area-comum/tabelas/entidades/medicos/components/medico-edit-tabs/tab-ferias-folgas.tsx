import { useState } from 'react'
import type { UseFormReturn } from 'react-hook-form'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { DateField } from '@/components/shared/date-field'
import { TimeField } from '@/components/shared/time-field'
import { Checkbox } from '@/components/ui/checkbox'
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogFooter,
} from '@/components/ui/dialog'
import { Plus, Trash2 } from 'lucide-react'
import type { MedicoDTO } from '@/types/dtos/saude/medicos.dtos'
import type { MedicoEditFormValues } from '../../types/medico-edit-form-types'
import {
  useGetFolgasMedicoByMedicoId,
  useCreateFolgasMedico,
  useDeleteFolgasMedico,
} from '../../queries/medicos-queries'
import type { FolgasMedicoDTO } from '@/types/dtos/saude/medicos.dtos'

function formatDateLabel(s: string) {
  if (!s) return '—'
  const d = new Date(s)
  if (Number.isNaN(d.getTime())) return s
  return d.toLocaleDateString('pt-PT', { day: '2-digit', month: '2-digit', year: 'numeric' })
}

function formatTimeDisplay(s: string | null | undefined): string {
  if (!s) return '—'
  const parts = s.split(':')
  if (parts.length >= 2) return `${parts[0]}:${parts[1]}`
  return s
}

export function TabFeriasFolgas({
  form,
  medico,
}: {
  form: UseFormReturn<MedicoEditFormValues>
  medico: MedicoDTO | undefined
}) {
  void form
  const medicoId = medico?.id ?? ''
  const hasMedico = !!medicoId

  const { data: response, isLoading } = useGetFolgasMedicoByMedicoId(medicoId)
  const createMutation = useCreateFolgasMedico(medicoId)
  const deleteMutation = useDeleteFolgasMedico(medicoId)

  const lista: FolgasMedicoDTO[] = response?.info?.data ?? []
  const [open, setOpen] = useState(false)
  const [dataDe, setDataDe] = useState('')
  const [dataAte, setDataAte] = useState('')
  const [todoDia, setTodoDia] = useState(false)
  const [mesInteiro, setMesInteiro] = useState(false)

  /** Quando "Todo o dia" está marcado, preenche automaticamente as horas (08:00-12:00 e 12:00-20:00) */
  const handleTodoDiaChange = (checked: boolean) => {
    setTodoDia(checked)
    if (checked) {
      setManhaInicio('08:00')
      setManhaFim('12:00')
      setTardeInicio('12:00')
      setTardeFim('20:00')
    }
  }
  const [manhaInicio, setManhaInicio] = useState('')
  const [manhaFim, setManhaFim] = useState('')
  const [tardeInicio, setTardeInicio] = useState('')
  const [tardeFim, setTardeFim] = useState('')

  const handleAdicionar = () => {
    if (!dataDe.trim() || !medicoId) return
    createMutation.mutate({
      medicoId,
      dataDe: dataDe.trim(),
      dataAte: dataAte.trim() || dataDe.trim(),
      todoDia,
      mesInteiro,
      manhaInicio: manhaInicio || undefined,
      manhaFim: manhaFim || undefined,
      tardeInicio: tardeInicio || undefined,
      tardeFim: tardeFim || undefined,
    })
    setDataDe('')
    setDataAte('')
    setTodoDia(false)
    setMesInteiro(false)
    setManhaInicio('')
    setManhaFim('')
    setTardeInicio('')
    setTardeFim('')
    setOpen(false)
  }

  const handleRemover = (id: string) => {
    deleteMutation.mutate(id)
  }

  if (!hasMedico) {
    return (
      <div className='rounded-md border border-dashed bg-muted/30 p-6 text-center text-sm text-muted-foreground'>
        Grave o médico primeiro para poder definir férias e folgas.
      </div>
    )
  }

  return (
    <div className='space-y-6'>
      <section>
        <div className='flex items-center justify-between gap-4 mb-3'>
          <h3 className='text-sm font-semibold text-muted-foreground border-b pb-1.5'>
            Férias / Folgas
          </h3>
          <Button
            type='button'
            variant='outline'
            size='sm'
            className='h-8'
            onClick={() => setOpen(true)}
            disabled={createMutation.isPending}
          >
            <Plus className='h-4 w-4 mr-2' />
            Adicionar férias
          </Button>
        </div>

        {isLoading ? (
          <div className='rounded-md border border-dashed bg-muted/30 p-6 text-center text-sm text-muted-foreground'>
            A carregar…
          </div>
        ) : lista.length === 0 ? (
          <div className='rounded-md border border-dashed bg-muted/30 p-6 text-center text-sm text-muted-foreground'>
            Nenhuma férias ou folga definida. Use o botão acima para adicionar.
          </div>
        ) : (
          <div className='rounded-md border overflow-hidden'>
            <table className='w-full text-sm'>
              <thead>
                <tr className='border-b bg-muted/50'>
                  <th className='text-left p-2 font-medium'>Data de</th>
                  <th className='text-left p-2 font-medium'>Data até</th>
                  <th className='text-left p-2 font-medium'>Todo o dia</th>
                  <th className='text-left p-2 font-medium'>Mês inteiro</th>
                  <th className='text-left p-2 font-medium'>Manhã</th>
                  <th className='text-left p-2 font-medium'>Tarde</th>
                  <th className='w-10 p-2'></th>
                </tr>
              </thead>
              <tbody>
                {lista.map((item) => (
                  <tr key={item.id} className='border-b last:border-0'>
                    <td className='p-2'>{formatDateLabel(item.dataDe)}</td>
                    <td className='p-2'>{formatDateLabel(item.dataAte)}</td>
                    <td className='p-2'>{item.todoDia ? 'Sim' : 'Não'}</td>
                    <td className='p-2'>{item.mesInteiro ? 'Sim' : 'Não'}</td>
                    <td className='p-2'>
                      {item.manhaInicio || item.manhaFim
                        ? `${formatTimeDisplay(item.manhaInicio)} - ${formatTimeDisplay(item.manhaFim)}`
                        : '—'}
                    </td>
                    <td className='p-2'>
                      {item.tardeInicio || item.tardeFim
                        ? `${formatTimeDisplay(item.tardeInicio)} - ${formatTimeDisplay(item.tardeFim)}`
                        : '—'}
                    </td>
                    <td className='p-2'>
                      <Button
                        type='button'
                        variant='ghost'
                        size='icon'
                        className='h-7 w-7 text-destructive hover:text-destructive'
                        onClick={() => handleRemover(item.id)}
                        title='Remover'
                        disabled={deleteMutation.isPending}
                      >
                        <Trash2 className='h-3.5 w-3.5' />
                      </Button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </section>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className='sm:max-w-md'>
          <DialogHeader>
            <DialogTitle>Adicionar férias</DialogTitle>
          </DialogHeader>
          <div className='grid gap-4 py-4'>
            <div className='grid grid-cols-2 gap-4'>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Data de</label>
                <DateField
                  className='h-8'
                  value={dataDe}
                  onChange={setDataDe}
                />
              </div>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Data até</label>
                <DateField
                  className='h-8'
                  value={dataAte}
                  onChange={setDataAte}
                />
              </div>
            </div>
            <div className='flex flex-wrap gap-6'>
              <label className='flex items-center space-x-2 cursor-pointer'>
                <Checkbox
                  checked={todoDia}
                  onCheckedChange={(c) => handleTodoDiaChange(!!c)}
                />
                <span className='text-sm font-normal'>Todo o dia</span>
              </label>
              <label className='flex items-center space-x-2 cursor-pointer'>
                <Checkbox
                  checked={mesInteiro}
                  onCheckedChange={(c) => setMesInteiro(!!c)}
                />
                <span className='text-sm font-normal'>Definir mês inteiro</span>
              </label>
            </div>
            <div className='grid grid-cols-2 gap-4'>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Manhã Início</label>
                <TimeField
                  className='h-8'
                  value={manhaInicio}
                  onChange={setManhaInicio}
                />
              </div>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Manhã Fim</label>
                <TimeField
                  className='h-8'
                  value={manhaFim}
                  onChange={setManhaFim}
                />
              </div>
            </div>
            <div className='grid grid-cols-2 gap-4'>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Tarde Início</label>
                <TimeField
                  className='h-8'
                  value={tardeInicio}
                  onChange={setTardeInicio}
                />
              </div>
              <div className='grid gap-2'>
                <label className='text-sm font-medium'>Tarde Fim</label>
                <TimeField
                  className='h-8'
                  value={tardeFim}
                  onChange={setTardeFim}
                />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button type='button' variant='outline' onClick={() => setOpen(false)}>
              Cancelar
            </Button>
            <Button
              type='button'
              onClick={handleAdicionar}
              disabled={!dataDe.trim() || createMutation.isPending}
            >
              Adicionar
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  )
}
