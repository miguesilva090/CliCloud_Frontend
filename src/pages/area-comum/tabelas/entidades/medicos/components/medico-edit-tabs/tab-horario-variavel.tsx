...
import {
  useGetHorarioMedicoVariavelByMedicoId,
  useCreateHorarioMedicoVariavel,
  useDeleteHorarioMedicoVariavel,
} from '../../queries/medicos-queries'
import type { HorarioMedicoVariavelDTO } from '@/types/dtos/saude/medicos.dtos'
...
export function TabHorarioVariavel({
  form,
  medico,
}: {
  ...
  const hasMedico = !!medicoId

  const { data: response, isLoading } = useGetHorarioMedicoVariavelByMedicoId(medicoId)
  const createMutation = useCreateHorarioMedicoVariavel(medicoId)
  const deleteMutation = useDeleteHorarioMedicoVariavel(medicoId)

  const lista: HorarioMedicoVariavelDTO[] = response?.info?.data ?? []
  ...
  if (!hasMedico) {
    return (
      <div className='rounded-md border border-dashed bg-muted/30 p-6 text-center text-sm text-muted-foreground'>
        Grave o médico primeiro para poder definir horários variáveis.
      </div>
    )
  }
              ...
              <tbody>
                {lista.map((item) => (
                  <tr key={item.id} className='border-b last:border-0'>
                    ...
                    <td className='p-2'>{formatTimeDisplay(item.manhaInicio)}</td>
                    <td className='p-2'>{formatTimeDisplay(item.manhaFim)}</td>
                    <td className='p-2'>{formatTimeDisplay(item.tardeInicio)}</td>
                    <td className='p-2'>{formatTimeDisplay(item.tardeFim)}</td>
                    <td className='p-2'>
                      <Button
                        type='button'
                        variant='ghost'
