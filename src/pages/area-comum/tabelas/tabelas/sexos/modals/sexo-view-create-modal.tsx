...
interface SexoViewCreateModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  mode: ModalMode
  viewData: SexoTableDTO | null
  onSuccess?: () => void
}
...
export function SexoViewCreateModal({
  open,
  onOpenChange,
  mode,
  viewData,
  onSuccess,
}: SexoViewCreateModalProps) {
    ...
    try {
      ...
      const response =
        isEdit && viewData && editId
          ? await client.updateSexo(editId, body)
          : await client.createSexo(body)

      if (response.info.status === ResponseStatus.Success) {
        toast.success(isEdit ? 'Sexo atualizado com sucesso.' : 'Sexo criado com sucesso.')
        onOpenChange(false)
        onSuccess?.()
      } else {
        const msg =
          response.info.messages?.['$']?.[0] ??
          (isEdit ? 'Falha ao atualizar Sexo.' : 'Falha ao criar Sexo.')
        toast.error(msg)
      }
    } catch (error: unknown) {
      const err = error as { message?: string }
      toast.error(err?.message ?? 'Ocorreu um erro ao guardar o Sexo.')
    }
  }

  const title =
    mode === 'view' ? 'Sexo' : mode === 'edit' ? 'Editar Sexo' : 'Adicionar Sexo'
      ...
      <DialogContent className='sm:max-w-md'>
        <DialogHeader>
          ...
          <DialogDescription className='sr-only'>
            Formulário para ver, criar ou editar sexo (descrição).
          </DialogDescription>
        </DialogHeader>
        <div className='grid gap-4 py-4'>
          <div className='grid gap-2'>
            ...
            <Input
              ...
              maxLength={80}
              placeholder='Descrição (ex: Masculino, Feminino, Indefinido)...'
              onChange={(e) =>
