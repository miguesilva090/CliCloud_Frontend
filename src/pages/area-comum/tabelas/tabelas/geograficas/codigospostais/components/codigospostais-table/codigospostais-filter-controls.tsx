...
import { filterFields } from '@/pages/area-comum/tabelas/tabelas/geograficas/codigospostais/components/codigospostais-table/codigospostais-constants'
...
export function CodigosPostaisFilterControls({
  table,
  columns,
}: BaseFilterControlsProps<CodigoPostalTableDTO>) {
  const windowId = useCurrentWindowId()
  const { filters } = useWindowPageState(windowId)
  const { filterValues, handleFilterChange } = useFilterState(table, filters)

  const renderFilterInput = (
    column: ColumnDef<CodigoPostalTableDTO, unknown>
  ) => {
