...
import { filterFields } from '@/pages/area-comum/tabelas/tabelas/geograficas/freguesias/components/freguesias-table/freguesias-constants'
...
export function FreguesiasFilterControls({
  table,
  columns,
}: BaseFilterControlsProps<FreguesiaTableDTO>) {
  const windowId = useCurrentWindowId()
  const { filters } = useWindowPageState(windowId)
  const { filterValues, handleFilterChange } = useFilterState(table, filters)

  const renderFilterInput = (column: ColumnDef<FreguesiaTableDTO, unknown>) => {
