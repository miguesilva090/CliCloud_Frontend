...
import { filterFields } from '@/pages/area-comum/tabelas/tabelas/geograficas/ruas/components/ruas-table/ruas-constants'
...
export function RuasFilterControls({
  table,
  columns,
}: BaseFilterControlsProps<RuaTableDTO>) {
  const windowId = useCurrentWindowId()
  const { filters } = useWindowPageState(windowId)
  const { filterValues, handleFilterChange } = useFilterState(table, filters)

  const renderFilterInput = (column: ColumnDef<RuaTableDTO, unknown>) => {
