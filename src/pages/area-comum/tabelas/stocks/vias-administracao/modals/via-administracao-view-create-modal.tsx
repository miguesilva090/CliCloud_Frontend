...
interface ViaAdministracaoViewCreateModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  mode: ModalMode
  viewData: ViaAdministracaoTableDTO | null
  onSuccess?: () => void
}
...
export function ViaAdministracaoViewCreateModal({
  open,
  onOpenChange,
  mode,
  viewData,
  onSuccess,
}: ViaAdministracaoViewCreateModalProps) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      const client = ViaAdministracaoService()
      ...
      const response =
        isEdit && viewData && editId
          ? await client.updateViaAdministracao(editId, { ...body, id: editId })
          : await client.createViaAdministracao(body)

      if (response.info.status === ResponseStatus.Success) {
        toast.success(
          isEdit
            ? 'Via de Administração atualizada com sucesso.'
            : 'Via de Administração criada com sucesso.',
        )
        onOpenChange(false)
        onSuccess?.()
      } else {
        const msg =
          ...
          (isEdit
            ? 'Falha ao atualizar Via de Administração.'
            : 'Falha ao criar Via de Administração.')
        toast.error(msg)
      }
    } catch (error: unknown) {
      const err = error as { message?: string }
      toast.error(err?.message ?? 'Ocorreu um erro ao guardar a Via de Administração.')
    }
  }
      ...
      <DialogContent className='sm:max-w-md'>
        <DialogHeader>
          ...
          <DialogDescription className='sr-only'>
            Formulário para ver, criar ou editar via de administração.
          </DialogDescription>
        </DialogHeader>
